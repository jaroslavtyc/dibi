...
*/ // note: this can change $this->args & $this->cursor & ...
						. preg_replace_callback('/(?=[`[\'":%?])(?:`(.+?)`|\[(.+?)\]|(\')((?:\'\'|[^\'])*)\'|(")((?:""|[^"])*)"|(\'|")|:(\S*?:)([a-zA-Z0-9._]?)|%([a-zA-Z~][a-zA-Z0-9~]{0,5})|(\?))/s', array($this, 'cb'), substr($arg, $toSkip));
					if (preg_last_error()) {
	...
	public function formatValue($value, $modifier) {
		...
		if ($modifier) {
			...
			switch ($modifier) {
				...
				case 'sn':
					return $value == '' ? 'NULL' : $this->driver->escape($value, pribi::TEXT); // notice two equal signs
				...
				case 'ex':
				case 'sql': // preserve as pribi-SQL  (TODO: leave only %ex)
					$value = (string) $value;
					// speed-up - is regexp required?
		...
		if (is_string($value)) {
			return $this->driver->escape($value, pribi::TEXT);
		} elseif (is_int($value)) {
			return (string) $value;
		} elseif (is_float($value)) {
			return rtrim(rtrim(number_format($value, 10, '.', ''), '0'), '.');
		} elseif (is_bool($value)) {
			return $this->driver->escape($value, pribi::BOOL);
		} elseif ($value === NULL) {
			return 'NULL';
		} elseif ($value instanceof DateTime) {
			return $this->driver->escape($value, pribi::DATETIME);
	...
	private function cb($matches) {
		...
		if ($matches[1]) { // SQL identifiers: `ident`
			...
			return $this->driver->escape(str_replace("''", "'", $matches[4]), pribi::TEXT);
		} elseif ($matches[5]) { // SQL strings: "..."
			return $this->driver->escape(str_replace('""', '"', $matches[6]), pribi::TEXT);
	...
	public function delimite($value) {
		...
		foreach ($parts as & $v) {
			if ($v !== '*') {
				$v = $this->driver->escape($v, pribi::IDENTIFIER);
			}
		}
