...
/**
 * The pribi reflector for MySQL databases.
 *
 * @author     David Grudl
...
class DibiMySqlReflector extends DibiObject implements IDibiReflector
{
	...
	{
		/*$table = $this->escape($table, pribi::TEXT);
		$this->query("
			SELECT *
			FROM INFORMATION_SCHEMA.COLUMNS
			WHERE TABLE_NAME = $table AND TABLE_SCHEMA = DATABASE()
		");*/
		$res = $this->driver->query("SHOW FULL COLUMNS FROM {$this->driver->escape($table, pribi::IDENTIFIER)}");
		$columns = array();
	...
	public function getIndexes($table)
	{
		...
		");*/
		$res = $this->driver->query("SHOW INDEX FROM {$this->driver->escape($table, pribi::IDENTIFIER)}");
		$indexes = array();
		while ($row = $res->fetch(TRUE)) {
