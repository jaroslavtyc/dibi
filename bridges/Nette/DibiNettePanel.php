...
/**
 * Pribi panel for Nette\Diagnostics.
 *
 * @author     David Grudl
 * @package    pribi\nette
 */
class DibiNettePanel extends DibiObject implements IBarPanel
{
	...
	public static function renderException($e)
	{
		if ($e instanceof DibiException && $e->getSql()) {
			return array(
				'tab' => 'SQL',
				'panel' => pribi::dump($e->getSql(), TRUE),
			);
		}
	}
	...
	public function getPanel()
	{
		...
		foreach ($this->events as $event) {
			...
			if ($this->explain && $event->type === DibiEvent::SELECT) {
				try {
					$backup = array($event->connection->onEvent, pribi::$numOfQueries, pribi::$totalTime);
					$event->connection->onEvent = NULL;
					$cmd = is_string($this->explain) ? $this->explain : ($event->connection->getConfig('driver') === 'oracle' ? 'EXPLAIN PLAN' : 'EXPLAIN');
					$explain = pribi::dump($event->connection->nativeQuery("$cmd $event->sql"), TRUE);
				} catch (DibiException $e) {}
				list($event->connection->onEvent, pribi::$numOfQueries, pribi::$totalTime) = $backup;
			}
			...
			$s .= '</td><td class="nette-DibiProfiler-sql">' . pribi::dump(strlen($event->sql) > self::$maxLength ? substr($event->sql, 0, self::$maxLength) . '...' : $event->sql, TRUE);
			if ($explain) {
