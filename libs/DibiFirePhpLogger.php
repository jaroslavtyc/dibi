...
class DibiFirePhpLogger extends DibiObject {
	...
	public function logEvent(DibiEvent $event) {
		...
		header('X-Wf-pribi-Structure-1: http://meta.firephp.org/Wildfire/Structure/FirePHP/FirebugConsole/0.1');

		$payload = json_encode(array(array('Type' => 'TABLE', 'Label' => 'pribi profiler (' . $this->numOfQueries . ' SQL queries took ' . sprintf('%0.3f', $this->totalTime * 1000) . ' ms)',), self::$fireTable,));
		foreach (str_split($payload, 4990) as $num => $s) {
			$num++;
			header("X-Wf-pribi-1-1-d$num: |$s|\\"); // protocol-, structure-, plugin-, message-index
		}
		header("X-Wf-pribi-1-1-d$num: |$s|");
	}
}
